# RTWrapper generated zero-parameter variant for /reload.
# NOTE: /reload is a dedicated-server-console-only command in vanilla Minecraft
...
tellraw @a[tag=rtwrapper.debug] [{"text":"[RTWrapper] ","color":"red"},{"text":"/reload cannot run from a datapack function (server-console-only command).","color":"gray"}]

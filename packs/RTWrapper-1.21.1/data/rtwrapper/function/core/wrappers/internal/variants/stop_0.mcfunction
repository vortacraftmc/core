# RTWrapper generated zero-parameter variant for /stop.
# NOTE: /stop is a dedicated-server-console-only command in vanilla Minecraft
...
tellraw @a[tag=rtwrapper.debug] [{"text":"[RTWrapper] ","color":"red"},{"text":"/stop cannot run from a datapack function (server-console-only command).","color":"gray"}]

# RTWrapper generated zero-parameter variant for /banlist.
# NOTE: /banlist is a dedicated-server-console-only command in vanilla Minecraft
...
tellraw @a[tag=rtwrapper.debug] [{"text":"[RTWrapper] ","color":"red"},{"text":"/banlist cannot run from a datapack function (server-console-only command).","color":"gray"}]

# Load a batch slot (1..9) into storage tunnelscript:in commands[].
# Input: {slot:1}
...
execute if score #batch_ok tunnelscript.vars matches 1 run tellraw @s [{"text":"[TunnelScript] loaded batch slot ","color":"aqua"},{"score":{"name":"#batch_slot","objective":"tunnelscript.vars"},"color":"white"}]
execute unless score #batch_ok tunnelscript.vars matches 1 run tellraw @s {"text":"[TunnelScript] batch load failed: slot is empty or invalid","color":"red"}
